...
mudSetup.network.components.Configuration.update$.subscribe(async (update) => {
    ...
    const { calculateMapHash, zkDig, verifyDig, solidityVerifier, formatProof } = await zkFunctions(width, height);
    ...
    const createGame = function () {
        ...
        const mapWithBorders = makeMapBorders(map);
        return {
            map,
            mapWithBorders,
    ...
    const makeMapBorders = function (map) {
    ...
    mudSetup.network.components.PendingGame.update$.subscribe(async (update) => {
        ...
        const tx = await mudSetup.network.worldContract.write.app__newGameResponse([newGameAddress, gameHash]);
    ...
    mudSetup.network.components.PendingDig.update$.subscribe(async (update) => {
        ...
        if (!Object.values(update.value)[0]?.wantsDig)
            return;
        const digResult = zkDig(gamesInProgress[update?.entity].mapWithBorders, Object.values(update.value)[0]?.x, Object.values(update.value)[0]?.y);
        //        const formattedResult = formatProof(digResult)[0]
        //        console.log(formattedResult)
        const tx = await mudSetup.network.worldContract.write.app__digResponse([update?.entity, digResult.inputs[0], digResult.inputs[1], digResult.inputs[10]]);
        await mudSetup.network.waitForTransaction(tx);
    });
});
