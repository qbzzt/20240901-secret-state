...
export const zkFunctions = async (width, height) => {
    ...
    const joinHashArray = function (arr) {
        return "0x" + arr.map(x => x.slice(2)).reduce((a, b) => a + b);
    };
    const calculateMapHash = function (hashMe) {
        return joinHashArray(JSON.parse(zokrates.computeWitness(hashCompiled, [hashMe]).output));
    };
    // Dig and return a zero knowledge proof of the result
    // (server-side code)
    const zkDig = function (map, x, y) {
        if (x < 0 || x >= width || y < 0 || y >= height)
            throw new Error("Trying to dig outside the map");
        const runResults = zokrates.computeWitness(digCompiled, [map, `${x}`, `${y}`]);
        const proof = zokrates.generateProof(digCompiled.program, runResults.witness, proverKey);
        return proof;
    };
    // Verify a dig's results (client-side code)
    const verifyDig = function (hashOfMap, digResultProof) {
        const hashInProof = "0x" + digResultProof.inputs.slice(2, -1)
            .map((x) => x.slice(-8)).reduce((a, b) => a + b);
    ...
    const formatProof = (proof) => zokrates.utils.formatProof(proof);
    return {
        zkDig,
        verifyDig,
        calculateMapHash,
...
//# sourceMappingURL=zero-knowledge.js.map